...
<nav x-data="{ mobileMenuOpen: false, userOpen: false }" class="bg-white shadow-sm fixed w-full top-0 z-50">
        ...
        <div class="flex justify-between items-center h-16">
            ...
            <div class="hidden md:flex items-center space-x-4">
                ...
                <a href="{{ route('services.index') }}"
                   class="text-gray-600 hover:text-indigo-600 px-3 py-2 rounded-md text-sm font-medium transition">Find Services</a>
            ...
            <div class="hidden md:flex items-center space-x-4">
                @auth
                    ...
                    <a href="{{ route('favorites.index') }}" class="relative p-2 rounded-md hover:bg-gray-100 transition">
                        <svg class="w-5 h-5 text-red-500" fill="currentColor" viewBox="0 0 24 24">
                            <path stroke="none" d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/>
                        </svg>
                    </a>
                    ...
                    <div class="relative" x-data="{ userOpen: false }">
                        ...
                        <div x-show="userOpen" @click.away="userOpen = false"
                            ...
                            @if(auth()->user()->role === 'student')
                                <a href="{{ route('services.manage') }}" class="block px-4 py-2 hover:bg-gray-100 rounded-md">Service Management</a>
                                <a href="{{ route('service-requests.index') }}" class="block px-4 py-2 hover:bg-gray-100 rounded-md">Service Requests</a>
                                <a href="{{ route('services.applications.index') }}" class="block px-4 py-2 hover:bg-gray-100 rounded-md">Open Applications</a>
                            @endif

                            @if(auth()->user()->isCommunity())
                                <a href="{{ route('search.index') }}" class="block px-4 py-2 hover:bg-gray-100 rounded-md">Browse Services</a>
                                <a href="{{ route('favorites.index') }}" class="block px-4 py-2 hover:bg-gray-100 rounded-md">My Favorites</a>
                                <a href="{{ route('service-requests.index') }}" class="block px-4 py-2 hover:bg-gray-100 rounded-md">My Service Requests</a>
                                <a href="{{ route('services.applications.index') }}" class="block px-4 py-2 hover:bg-gray-100 rounded-md">My Applications</a>
                                <a href="{{ route('services.apply') }}" class="block px-4 py-2 hover:bg-gray-100 rounded-md">Request Custom Service</a>
                            @endif
                ...
                @else
                    <!-- Guest -->
                    <a href="{{ route('login') }}" class="text-gray-600 hover:text-indigo-600 px-3 py-2 rounded-md text-sm">Login</a>
    ...
    <div x-show="mobileMenuOpen" x-transition class="md:hidden bg-white border-t">
        <div class="px-4 pt-4 pb-6 space-y-1">
            <a href="{{ auth()->check() ? route('dashboard') : route('welcome') }}" class="block px-3 py-2 text-gray-700 hover:bg-gray-100 rounded-md">Home</a>
            <a href="{{ route('services.index') }}" class="block px-3 py-2 text-gray-700 hover:bg-gray-100 rounded-md">Find Services</a>
            <a href="{{ route('about') }}" class="block px-3 py-2 text-gray-700 hover:bg-gray-100 rounded-md">About Us</a>
            ...
            @auth
                <div class="border-t pt-4 space-y-1">
                    <a href="{{ route('chat.index') }}" class="block px-3 py-2 text-gray-700 hover:bg-gray-100 rounded-md">Messages</a>
                    <a href="{{ route('favorites.index') }}" class="block px-3 py-2 text-gray-700 hover:bg-gray-100 rounded-md">Favorites</a>
                    <a href="{{ route('service-requests.index') }}" class="block px-3 py-2 text-gray-700 hover:bg-gray-100 rounded-md">Service Requests</a>
                    <a href="{{ route('profile.edit') }}" class="block px-3 py-2 text-gray-700 hover:bg-gray-100 rounded-md">Profile</a>
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button type="submit" class="w-full text-left px-3 py-2 text-red-600 hover:bg-red-50 rounded-md">Sign Out</button>
                    </form>
                </div>
            @else
                <a href="{{ route('login') }}" class="block px-3 py-2 text-gray-700 hover:bg-gray-100 rounded-md">Login</a>
                <a href="{{ route('register') }}" class="block px-3 py-2 bg-indigo-600 text-white rounded-md text-center hover:bg-indigo-700">Sign Up</a>
            @endauth
        </div>
    </div>
</nav>
