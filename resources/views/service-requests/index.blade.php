<x-app-layout>
                        ...
                        <div id="pending-content" class="sr-status-tab-content {{ $defaultStatusTab === 'pending' ? '' : 'hidden' }}">
                            @if ($sentRequests->where('status', 'pending')->isEmpty())
                                <div class="text-center py-8">
                                    ...
                                    <div class="mt-6">
                                        <a href="{{ route('services.index') }}" class="inline-flex items-center px-4 py-2 shadow-sm text-sm font-medium rounded-md bg-custom-teal text-white hover:bg-teal-700 transition">
                                            Browse Services
                                        </a>
                                    </div>
                                </div>
    ...
    @foreach ($sentRequests->where('status', 'pending') as $request)
        ...
        <div class="group relative overflow-hidden rounded-xl border border-gray-200 bg-gradient-to-br from-white to-gray-50 p-6 transition-all duration-300 hover:shadow-lg hover:border-yellow-100">
            ...
            <div class="flex flex-col gap-6 md:flex-row">
                ...
                <div class="flex-1 space-y-4 pl-2">
                    ...
                    <div class="flex justify-between items-start">
                        <div>
                            <h4 class="text-xl font-bold text-gray-900 group-hover:text-yellow-600 transition-colors">
                                {{ optional($request->studentService)->title ?? 'Custom Request' }}
                            </h4>
                            <div class="mt-1 flex items-center gap-2 text-sm text-gray-500">
                                <span class="font-medium text-gray-700">Provider: {{ $request->provider->name }}</span>
                                <span class="text-gray-300">|</span>
                                <span class="text-xs">Sent {{ $request->created_at->diffForHumans() }}</span>
                            </div>
                        </div>
                        
                        <span class="inline-flex items-center rounded-full border border-yellow-200 bg-yellow-50 px-3 py-1 text-xs font-bold uppercase tracking-wide text-yellow-700 shadow-sm animate-pulse">
                            {{ $request->formatted_status }}
                        </span>
                    </div>

                    <div class="rounded-lg border border-gray-200 bg-white/50 p-4 shadow-sm backdrop-blur-sm">
                        <div class="mb-3 flex items-center justify-between border-b border-gray-100 pb-2">
                            <span class="text-sm font-bold text-indigo-900 uppercase tracking-wider">
                        ...
                        <div class="flex flex-wrap gap-2">
                            @if ($pkgDuration)
                                <span class="inline-flex items-center gap-1 rounded-md bg-gray-100 px-2.5 py-1 text-xs font-medium text-gray-700">
                                    ...
                                    {{ $pkgDuration }} Hour{{ $pkgDuration > 1 ? 's' : '' }}
                                </span>
                            @endif
                            ...
                            @if ($request->selected_dates)
                                <span class="inline-flex items-center gap-1 rounded-md bg-blue-50 px-2.5 py-1 text-xs font-medium text-blue-700">
                                    <svg class="h-3.5 w-3.5 text-blue-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
                    ...
                    @if ($request->message)
                        <div class="relative rounded-md bg-gray-50 p-3 pl-4 text-sm text-gray-600 italic">
                            <div class="absolute left-0 top-0 bottom-0 w-1 rounded-l-md bg-gray-300"></div>
                            "{{ $request->message }}"
                        </div>
                    @endif

                    <div class="pt-1">
                        <a href="{{ route('service-requests.show', $request) }}" class="group/link inline-flex items-center text-sm font-semibold text-yellow-600 hover:text-yellow-800">
                            View Full Details
                            <svg class="ml-1 h-4 w-4 transition-transform group-hover/link:translate-x-1" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 8l4 4m0 0l-4 4m4-4H3" /></svg>
                        </a>
                    </div>
                </div>

                <div class="flex min-w-[140px] flex-col justify-between gap-3 border-t border-gray-100 pt-4 md:border-l md:border-t-0 md:pl-6 md:pt-0">
                    <div class="space-y-2">
                        ...
                        <div id="in-progress-content" class="sr-status-tab-content {{ $defaultStatusTab === 'in-progress' ? '' : 'hidden' }}">
                            @if ($sentRequests->whereIn('status', ['accepted', 'in_progress'])->isEmpty())
                                <div class="text-center py-8">
                                    ...
                                    <p class="mt-1 text-sm text-gray-500">You have no active services at the moment.</p>
                                </div>
                            @else
                                <div class="space-y-6">
    @foreach ($sentRequests->whereIn('status', ['accepted', 'in_progress']) as $request)
        ...
        <div class="group relative overflow-hidden rounded-xl border border-gray-200 bg-gradient-to-br from-white to-gray-50 p-6 transition-all duration-300 hover:shadow-lg hover:border-blue-100">
            ...
            <div class="flex flex-col gap-6 md:flex-row">
                ...
                <div class="flex-1 space-y-4 pl-2">
                    ...
                    <div class="flex justify-between items-start">
                        <div>
                            ...
                            <div class="mt-1 flex items-center gap-2 text-sm text-gray-500">
                                <span class="font-medium text-gray-700">Provider: {{ $request->provider->name }}</span>
                                <span class="text-gray-300">|</span>
                                <span class="text-xs">Started {{ $request->updated_at->diffForHumans() }}</span>
                            </div>
                        </div>
                        
                        <span class="inline-flex items-center rounded-full border border-blue-200 bg-blue-50 px-3 py-1 text-xs font-bold uppercase tracking-wide text-blue-700 shadow-sm animate-pulse">
                    ...
                    <div class="pt-1">
                        <a href="{{ route('service-requests.show', $request) }}" class="group/link inline-flex items-center text-sm font-semibold text-blue-600 hover:text-blue-800">
                            View Full Details
                            <svg class="ml-1 h-4 w-4 transition-transform group-hover/link:translate-x-1" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 8l4 4m0 0l-4 4m4-4H3" /></svg>
                        </a>
                    </div>
                ...
                <div class="flex min-w-[140px] flex-col justify-center gap-3 border-t border-gray-100 pt-4 md:border-l md:border-t-0 md:pl-6 md:pt-0">
                    <div class="space-y-2">
                        <span class="block text-xs font-semibold uppercase text-gray-400">Actions</span>
                        ...
                        <div id="completed-content" class="sr-status-tab-content {{ $defaultStatusTab === 'completed' ? '' : 'hidden' }}">
                            @if ($sentRequests->whereIn('status', ['completed', 'cancelled', 'rejected'])->isEmpty())
                                <div class="text-center py-8">
                                    ...
                                    <p class="mt-1 text-sm text-gray-500">You have no completed or cancelled requests.</p>
                                </div>
                            @else
                               <div class="space-y-6">
    @foreach ($sentRequests->whereIn('status', ['completed', 'cancelled', 'rejected']) as $request)
        ...
        <div class="group relative overflow-hidden rounded-xl border border-gray-100 bg-white p-6 opacity-90 transition-all duration-300 hover:opacity-100 hover:shadow-md">
            ...
            <div class="flex flex-col gap-6 md:flex-row">
                ...
                <div class="flex-1 space-y-4 pl-3">
                    ...
                    <div class="flex justify-between items-start">
                        <div>
                            ...
                            <div class="mt-1 flex items-center gap-2 text-sm text-gray-400">
                                <span class="font-medium">Provider: {{ $request->provider->name }}</span>
                                <span class="text-gray-300">|</span>
                                <span class="text-xs">
                                    @if($request->status === 'completed')
                                        Completed {{ $request->updated_at->format('M j, Y') }}
                                    @else
                        ...
                        <span class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-bold uppercase tracking-wide {{ $request->status_color }}">
                            {{ $request->formatted_status }}
                        </span>
                    </div>

                    <div class="flex flex-wrap items-center gap-3 text-sm text-gray-500 bg-gray-50 rounded-lg p-3 border border-gray-100">
                         <div class="flex items-center gap-1.5">
                            <span class="font-medium">{{ ucfirst($request->selected_package ?? 'Custom') }}</span>
                        </div>
                        
                        @if ($request->offered_price)
                            <div class="flex items-center gap-1.5 text-gray-700">
                                <span class="font-bold">RM {{ number_format($request->offered_price, 2) }}</span>
                            </div>
                        @endif
                    ...
                    <div class="pt-1">
                        <a href="{{ route('service-requests.show', $request) }}" class="inline-flex items-center text-sm font-semibold text-gray-500 hover:text-indigo-600 transition-colors">
                ...
                <form id="reviewForm" class="space-y-4">
                    ...
                    <div>
                        ...
                        <div class="flex space-x-1 justify-center">
                            @for ($i = 1; $i <= 5; $i++)
                                <button type="button" onclick="setRating({{ $i }})" class="star-button text-3xl text-gray-300 hover:text-yellow-400 focus:outline-none transition-colors">★</button>
                            @endfor
                        </div>
                    ...
                    <div>
                        ...
                        <textarea id="comment" name="comment" rows="3" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500" placeholder="How was your experience?"></textarea>
                    </div>
                    ...
                    <div class="flex space-x-3 pt-4">
                        ...
                        <button type="submit" class="flex-1 px-4 py-2 border border-transparent rounded-md text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700">Submit</button>
                    </div>
                </form>
            </div>
        </div>
        ...
        function cancelRequest(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    .then(data => {
                        if(data.success) {
                            Swal.fire("Cancelled", "Your request has been cancelled.", "success").then(() => location.reload());
                        } else {
