...
@section('content')
    ...
    @if(request('search') || request('rating_range'))
        <a href="{{ route('admin.community.index', ['status' => request('status')]) }}" 
           class="px-3 py-2 text-gray-500 bg-gray-100 hover:bg-gray-200 rounded-lg text-sm transition" title="Clear Filters">
    ...
    @foreach ($communityUsers as $user)
        <tr class="border-b hover:bg-gray-50">
                ...
                <div class="flex flex-col items-center justify-center">
                    ...
                    @if($user->reviews_received_count > 0)
                        ...
                        <span class="text-xs text-gray-400 mt-0.5">
                            ({{ $user->reviews_received_count }} {{ Str::plural('review', $user->reviews_received_count) }})
                        </span>
                    @else
            ...
            <td class="py-3 px-4 text-sm text-center whitespace-nowrap">
                ...
                <div id="reviews-modal-{{ $user->id }}"
                    class="fixed inset-0 z-50 hidden text-left"
                        ...
                        <div class="flex min-h-full items-center justify-center p-4 text-center sm:p-0">
                            ...
                            <div class="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-2xl">
                                ...
                                <div class="bg-gray-50 px-4 py-3 sm:px-6 flex justify-between items-center border-b">
                                    ...
                                    <button type="button" onclick="closeReviewsModal('reviews-modal-{{ $user->id }}')" class="text-gray-400 hover:text-gray-500">
                                        <i class="fa-solid fa-xmark text-xl"></i>
                                    </button>
                                </div>

                                <div class="bg-white px-4 py-5 sm:p-6 max-h-[60vh] overflow-y-auto custom-scrollbar">

                                    {{-- Use eager loaded relationship 'reviewsReceived' --}}
                                    @if($user->reviewsReceived->isNotEmpty())
                                        <div class="space-y-6">
                                            @foreach($user->reviewsReceived as $review)
                                                <div class="flex gap-4 p-4 rounded-lg bg-gray-50 border border-gray-100">
                                                    ...
                                                    <div class="flex-1">
                                                        ...
                                                        <div class="flex items-center mb-2">
                                                            ...
                                                            @endfor
                                                            <span class="ml-2 text-xs font-medium text-gray-600">({{ $review->rating }}.0)</span>
                                                        </div>

                                                        @if($review->comment)
                                                            <p class="text-sm text-gray-700 italic">"{{ $review->comment }}"</p>
                                                        @else
                                                            <p class="text-sm text-gray-400 italic">No written comment.</p>
                                                        @endif

                                                        @if($review->reply)
                                                            <div class="mt-3 ml-2 pl-3 border-l-2 border-blue-300 bg-blue-50 p-2 rounded-r">
                                                                <p class="text-xs font-bold text-blue-800 mb-0.5">Reply:</p>
                                                                <p class="text-xs text-blue-900">{{ $review->reply }}</p>
                                    ...
                                    @else
                                        <div class="text-center py-8">
                                            <div class="inline-flex items-center justify-center w-12 h-12 rounded-full bg-gray-100 mb-3">
                                                <i class="fa-regular fa-comment-dots text-gray-400 text-xl"></i>
                                            </div>
                                            <h3 class="mt-2 text-sm font-medium text-gray-900">No reviews yet</h3>
                                            <p class="mt-1 text-sm text-gray-500">This user hasn't received any feedback.</p>
                                        </div>
                                    @endif
                                ...
                                <div class="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
                                    <button type="button" onclick="closeReviewsModal('reviews-modal-{{ $user->id }}')"
