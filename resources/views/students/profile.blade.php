                    ...
                    @if ($user->skills)
                        <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                            <h3 class="font-bold text-gray-900 mb-4 flex items-center gap-2">
                                <span class="bg-indigo-100 text-indigo-600 w-8 h-8 rounded-lg flex items-center justify-center"><i class="fa-solid fa-wand-magic-sparkles text-sm"></i></span>
                                Skills & Expertise
                            </h3>
                            <div class="flex flex-wrap gap-2">
                                @foreach (explode(',', $user->skills) as $skill)
                                    <span class="px-3 py-1.5 rounded-lg text-sm font-semibold bg-slate-50 text-slate-700 border border-slate-200">
                                        {{ trim($skill) }}
                                    </span>
                                @endforeach
                            </div>
                                    ...
                                    <div class="border-b border-gray-50 pb-8 last:border-0 last:pb-0">
                                        <div class="flex items-start gap-4">
                                            ...
                                            <div class="flex-1">
                                                ...
                                                {{-- Review Context --}}
                                                @if ($review->service)
                                                    <div class="mb-3 inline-flex items-center gap-1 px-2.5 py-1 rounded-md bg-slate-50 border border-slate-100 text-xs text-slate-500">
                                                        <span>Service:</span>
                                                        <a href="{{ route('student-services.show', $review->service->id) }}" class="font-semibold text-indigo-600 hover:underline">
                                                            {{ Str::limit($review->service->title, 40) }}
                                                        </a>
                                                    </div>
                                                @endif
