            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden mb-8">
                ...
                <div class="px-8 pb-8">
                    <div class="relative flex flex-col md:flex-row items-end -mt-12 mb-4">
                        <div class="relative">
                            <div
                                class="w-32 h-32 md:w-40 md:h-40 rounded-full border-4 border-white overflow-hidden bg-white shadow-md">
                                @if ($user->profile_photo_path)
                                    <img src="{{ asset('storage/' . $user->profile_photo_path) }}"
                                        alt="{{ $user->name }}" class="w-full h-full object-cover">
                                @else
                                    <div
                                        class="w-full h-full bg-slate-800 flex items-center justify-center text-white font-bold text-4xl">
                                        {{ substr($user->name, 0, 1) }}
                                    </div>
                                @endif
                            </div>
                            @if ($user->trust_badge)
                                <div class="absolute bottom-2 right-2 bg-blue-500 text-white p-1 rounded-full border-2 border-white"
                        ...
                        <div class="md:ml-6 mt-4 md:mt-0 flex-1">
                            <div class="flex flex-col md:flex-row md:items-center justify-between">
                                <div>
                                    ...
                                    <p class="text-gray-500 font-medium">{{ $user->faculty ?? 'General User' }}
                                        {{ $user->course ? '• ' . $user->course : '' }}</p>
                                    ...
                                    <div class="mt-2 flex items-center gap-3">
                                        ...
                                        @else
                                            <span
                                                class="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-50 text-red-700 border border-red-200">
                                        ...
                                        <div class="flex items-center gap-1 text-sm text-gray-600">
                                            ...
                                            <span
                                                class="font-bold">{{ number_format($user->average_rating ?? 0, 1) }}</span>
                                            <span class="text-gray-400">({{ $user->reviews_received_count ?? 0 }}
                                                reviews)</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="mt-4 md:mt-0">
                                    @auth
                                        ...
                                        @if ($viewer->id !== $user->id)
                                            <a href="{{ route('chat.request', ['user' => $user->id]) }}"
                                                class="inline-flex items-center px-6 py-3 bg-slate-900 hover:bg-slate-800 text-white font-semibold rounded-xl transition shadow-lg transform hover:-translate-y-0.5">
                                    ...
                                    @else
                                        <a href="{{ route('login') }}"
                                            class="inline-flex items-center px-6 py-3 bg-slate-900 hover:bg-slate-800 text-white font-semibold rounded-xl transition shadow-lg">
                                            Login to Contact
                                        </a>
                                    @endauth
                                </div>
                            ...
                            @if ($user->services->count() > 0)
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                    @foreach ($user->services as $service)
                                        <div
                                            class="bg-white rounded-2xl shadow-sm border border-gray-100 hover:shadow-md transition-all group overflow-hidden flex flex-col h-full">
                                            <a href="{{ route('student-services.show', $service) }}"
                                                ...
                                                @endif
                                                <div
                                                    class="absolute top-3 right-3 bg-white/90 backdrop-blur-sm px-2 py-1 rounded-md text-xs font-bold shadow-sm">
                                                    Starting at
                                                    RM{{ number_format($service->suggested_price ?? ($service->basic_price ?? 0), 0) }}
                                                </div>
                                            </a>

                                            <div class="p-5 flex flex-col flex-1">
                                                <a href="{{ route('student-services.show', $service) }}"
                                                    class="text-lg font-bold text-gray-900 hover:text-indigo-600 line-clamp-2 mb-2">
                                                    {{ $service->title }}
                                                </a>
                                                ...
                                                <div class="pt-4 border-t border-gray-50 mt-auto">
                                                    <a href="{{ route('student-services.show', $service) }}"
                                                        ...
                                                        View Details <svg class="w-4 h-4" fill="none"
                    ...
                    <div class="bg-white rounded-2xl p-6 md:p-8 shadow-sm border border-gray-100">
                        <div class="flex items-center justify-between mb-6">
                            ...
                            <div class="flex items-center gap-2">
                                <span
                                    class="text-2xl font-bold text-gray-900">{{ number_format($user->average_rating ?? 0, 1) }}</span>
                                    ...
                                    <div class="border-b border-gray-100 pb-6 last:border-0 last:pb-0">
                                        <div class="flex items-start gap-4">
                                            ...
                                            <div class="flex-1">
                                                <div class="flex justify-between items-start">
                                                    <div>
                                                        <h4 class="font-bold text-gray-900 text-sm">
                                                            {{ $review->reviewer->name ?? 'Anonymous' }}</h4>
                                                        <div class="flex text-yellow-400 text-xs mt-1">
                                                            @for ($i = 1; $i <= 5; $i++)
                                                                <svg class="w-3 h-3 {{ $i <= $review->rating ? '' : 'text-gray-200' }}"
                                                            ...
                                                            @endfor
                                                            <span
                                                                class="text-gray-400 ml-2">{{ $review->created_at->diffForHumans() }}</span>
                                                        </div>
                                                    </div>
                                                </div>
                                                ...
                                                @if ($review->service)
                                                    <div
                                                        class="mt-1 text-xs text-gray-500 bg-gray-50 inline-block px-2 py-1 rounded">
                                                        Review for: <a
                                                            href="{{ route('student-services.show', $review->service->id) }}"
                                                            class="text-indigo-600 hover:underline font-medium">{{ $review->service->title }}</a>
                                                    </div>
                                                @endif
                        ...
                        @else
                            <div class="text-center py-8">
                                ...
                                <p class="text-gray-500 text-sm">Be the first to hire and review {{ $user->name }}!
                                </p>
                            </div>
                        @endif
                    </div>
