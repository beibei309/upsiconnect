                    ...
                    <div x-show="showFilters" 
                        ...
                        <div>
                            ...
                            <select name="category_id" class="block w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                                ...
                                <option value="1" {{ $category_id == '1' ? 'selected' : '' }}>Tutoring</option>
                                <option value="2" {{ $category_id == '2' ? 'selected' : '' }}>Design</option>
                                <option value="3" {{ $category_id == '3' ? 'selected' : '' }}>Programming</option>
                            </select>
                        </div>
                    ...
                    <div class="flex flex-wrap gap-2 mt-4 pt-4 border-t border-gray-200">
                        ...
                        @if($q)
                            <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-indigo-100 text-indigo-800">
                                Search: "{{ $q }}"
                                <a href="{{ route('search.index', array_filter(['category_id' => $category_id, 'min_rating' => $min_rating, 'available' => $available])) }}" class="ml-1 text-indigo-600 hover:text-indigo-500">
                        ...
                        @if($category_id)
                            <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-green-100 text-green-800">
                                Category
                                <a href="{{ route('search.index', array_filter(['q' => $q, 'min_rating' => $min_rating, 'available' => $available])) }}" class="ml-1 text-green-600 hover:text-green-500">
                                    <svg class="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
                                        <path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"></path>
                                    </svg>
                                </a>
                        ...
                        @if($min_rating)
                            <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800">
                                {{ $min_rating }}+ Stars
                                <a href="{{ route('search.index', array_filter(['q' => $q, 'category_id' => $category_id, 'available' => $available])) }}" class="ml-1 text-yellow-600 hover:text-yellow-500">
                                    <svg class="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
                                        <path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"></path>
                                    </svg>
                                </a>
                            ...
                            <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-shadow group">
                                ...
                                <div class="p-6 pb-4">
                                    <div class="flex items-start justify-between">
                                            ...
                                            <div>
                                                ...
                                                <div class="flex items-center space-x-2 mt-1">
                                                    ...
                                                    <div class="flex items-center">
                                                        <div class="flex items-center">
                                                            @for($i = 1; $i <= 5; $i++)
                                                                <svg class="w-3 h-3 {{ $i <= ($service->user->average_rating ?? 0) ? 'text-yellow-400' : 'text-gray-300' }}" fill="currentColor" viewBox="0 0 20 20">
                                        ...
                                        <!-- Availability Status -->
                                        <div class="flex items-center">
                                            <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium {{ $service->user->is_available ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                                <div class="w-1.5 h-1.5 rounded-full {{ $service->user->is_available ? 'bg-green-400' : 'bg-red-400' }} mr-1"></div>
                                ...
                                <div class="px-6 pb-6">
                                    <a href="{{ route('students.profile', $service->user) }}" 
                                       class="w-full inline-flex items-center justify-center px-4 py-2 border border-transparent rounded-lg text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-colors group-hover:bg-indigo-700">
                                        View Profile
