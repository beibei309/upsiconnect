...
<body class="antialiased text-slate-800">
        ...
        <div class="bg-white border-b border-gray-200 pt-24 pb-12">
            <div class="max-w-7xl mx-auto px-6">
                ...
                <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        <h1 class="text-3xl md:text-4xl font-bold text-slate-900 tracking-tight mb-2">Find Student Services</h1>
                        <p class="text-slate-500 text-lg">Browse services offered by UPSI students. Fast, reliable, and affordable.</p>
                    </div>
                    </div>
            </div>
        </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 sticky top-24">
                ...
                <main class="lg:col-span-9">
                            ...
                            <div class="flex gap-2">
                                ...
                                <div class="relative min-w-[130px]">
                                    <select name="available_only" onchange="this.form.submit()"
                                        ...
                                        <option value="1" {{ request('available_only') == '1' ? 'selected' : '' }}>Available Only</option>
                                        <option value="0" {{ request('available_only') == '0' ? 'selected' : '' }}>Busy / Away</option>
                                    </select>
                                    <div class="absolute inset-y-0 right-0 flex items-center px-2 pointer-events-none text-gray-500">
                                        <i class="fas fa-chevron-down text-xs"></i>
                                    </div>
                                ...
                                <button type="submit" class="hidden md:block bg-indigo-600 hover:bg-indigo-700 text-white px-5 rounded-xl transition shadow-md hover:shadow-lg">
                                    Search
                                </button>
                            </div>
                        </form>
                    ...
                    <div class="space-y-6">
                        @foreach ($services as $service)
                            <div class="group bg-white rounded-2xl border border-gray-100 p-4 sm:p-5 shadow-sm hover:shadow-xl transition-all duration-300 relative overflow-hidden">
                                ...
                                <div class="flex flex-col sm:flex-row gap-6">
                                    ...
                                    <div class="sm:w-64 h-56 sm:h-auto flex-shrink-0 relative rounded-xl overflow-hidden bg-gray-100">
                                        ...
                                        @if ($service->basic_price)
                                            <div class="sm:hidden absolute bottom-3 right-3 bg-indigo-600 text-white px-3 py-1 rounded-lg text-sm font-bold shadow-md">
                                    ...
                                    <div class="flex-1 flex flex-col justify-between">
                                        <div>
                                            <div class="flex justify-between items-start mb-2">
                                                <div class="flex items-center gap-2">
                                                    <span class="text-xs font-medium text-gray-400 flex items-center gap-1">
                                                        <i class="far fa-clock"></i> {{ $service->created_at->diffForHumans() }}
                                                    </span>
                                                    <span class="inline-block w-1 h-1 rounded-full bg-gray-300"></span>
                                                    <span class="text-xs font-medium {{ $service->user->is_available ? 'text-green-600 bg-green-50' : 'text-red-600 bg-red-50' }} px-2 py-0.5 rounded-full border {{ $service->user->is_available ? 'border-green-100' : 'border-red-100' }}">
                                                        {{ $service->user->is_available ? 'Available' : 'Busy' }}
                                                    </span>
                                                </div>
                                                
                                                <div class="flex items-center gap-2">
                                                    <button type="button" onclick="handleFavourite({{ $service->id }}, {{ auth()->check() ? 'true' : 'false' }})"
                                                        class="w-8 h-8 flex items-center justify-center rounded-full hover:bg-gray-100 transition text-gray-400 hover:text-red-500" title="Add to favourites">
                                                        <i id="heart-{{ $service->id }}" class="{{ auth()->check() && $service->is_favourited ? 'fas text-red-500' : 'far' }} fa-heart text-lg"></i>
                                                    </button>
                                                    <button type="button" onclick="handleShare(this)" data-url="{{ route('services.details', $service->id) }}" 
                                                        class="w-8 h-8 flex items-center justify-center rounded-full hover:bg-gray-100 transition text-gray-400 hover:text-indigo-600" title="Share">
                                            ...
                                            <div class="flex items-center gap-1 mb-3">
                                                ...
                                                <span class="font-bold text-slate-800 text-sm">{{ number_format($service->user->average_rating ?? 0, 1) }}</span>
                                                <span class="text-slate-400 text-sm">({{ $service->user->reviewsReceived()->count() }} reviews)</span>
                                            </div>
                                        ...
                                        <div class="flex items-center justify-between border-t border-gray-100 pt-4 mt-auto">
                                            ...
                                            <div class="flex items-center gap-4">
                                                ...
                                                <div class="text-right hidden sm:block">
                                                    ...
                                                    <p class="text-lg font-bold text-indigo-600">RM{{ number_format($service->basic_price, 2) }}</p>
                                                </div>
                                                @endif
        ...
        <div id="shareModal" class="fixed inset-0 z-50 flex items-center justify-center opacity-0 pointer-events-none transition-opacity duration-300">
            <div class="absolute inset-0 bg-black/60 backdrop-blur-sm" onclick="closeShareModal()"></div>
            
            <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 relative transform scale-95 transition-transform duration-300 mx-4">
                <button onclick="closeShareModal()" class="absolute top-4 right-4 text-gray-400 hover:text-gray-600 transition">
                ...
                <div class="relative">
                    <input type="text" id="shareLinkInput" class="w-full bg-gray-50 border border-gray-200 text-gray-600 text-sm rounded-xl px-4 py-3 pr-24 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none" readonly>
                    <button onclick="copyShareLink()" class="absolute right-1 top-1 bottom-1 bg-indigo-600 hover:bg-indigo-700 text-white px-4 rounded-lg text-sm font-medium transition">
                        Copy
                    </button>
                </div>
                <p id="copyMessage" class="text-green-600 text-xs font-semibold text-center mt-3 h-4 opacity-0 transition-opacity">Link Copied Successfully!</p>
            </div>
        </div>
